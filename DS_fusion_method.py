...
def DS_fusion_method(data):
    ...
    print('经典DS理论算出结果为 ' + str(fusion))
    return fusion, K

def use_DS_method_of_sun(data, label):
    ...
    fusion_all = pd.DataFrame(data=fusion_all, columns=label, index=['ProcessData', 'Alert', 'Fusion'])
    print(fusion_all)
    return fusion_all
...
if __name__ == '__main__':
    #测试data
    data = [[0.5, 0.2, 0.3]]
    data.append([0, 0.9, 0.1])
    # data.append([0.6, 0.1, 0.3])
    data = np.array(data)
